...
def probe_positions(probe_distribution, geometry_params):
    ...
    elif (distribution_type == 'mesh_grid_points'):
        
        loaded_position_probes = np.load("mesh_points/points.npy")[...,:-1]
        list_position_probes = []
        for crrt_xy in range(loaded_position_probes.shape[0]):
                list_position_probes.append(loaded_position_probes[crrt_xy])  # Append (x,y) pairs np array 
