...
def train(agent, env, replay, logger, args):
  ...
  print("len replay in driver: ", len(replay))
  print("max() in driver: ", max(args.batch_steps, args.train_fill))
  while len(replay) < max(args.batch_steps, args.train_fill):
    # print("inside while loop")  #added by Priyam
    print("len replay in driver: ", len(replay))
    driver(random_agent.policy, steps=100)
  logger.add(metrics.result())
  logger.write()
  
  print('Done Prefill train dataset.')
...
#   print("In embodied/run/train.py", flush = True)
...
#         'reward_rate': (np.abs(ep['reward']) >= 0.5).mean(),
#     }, prefix='episode')
#     print(f'Episode has {length} steps and return {score:.1f}.', flush = True)
#     stats = {}
#     for key in args.log_keys_video:
#       if key in ep:
#         stats[f'policy_{key}'] = ep[key]
...
#   print('Prefill train dataset.', flush = True)
#   random_agent = embodied.RandomAgent(env.act_space)
#   while len(replay) < max(args.batch_steps, args.train_fill):
#     driver(random_agent.policy, steps=100)
#   logger.add(metrics.result())
