...
def eval_only_rollout(agent, env, logger, args):
  ...
  def eval_rollout_log(tran):
    # print(tran.keys())
    # print(tran["action"])
    logger.add({'reward': tran['reward'], 'action': tran["action"], 'state': tran["vector"]}, prefix='eval_rollout_episode')
    metrics.add({'reward': tran['reward'], 'action': tran["action"], 'state': tran["vector"]}, prefix='eval_episode')
