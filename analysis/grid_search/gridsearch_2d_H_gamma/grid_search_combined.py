...
if __name__ == '__main__':
    ...
    # Contour plot
    plt.contourf(param1, param2, AUCS_mean.T, levels=15)
    # plt.title(f"AUCS")
    # plt.colorbar(ticks=[f"{x:.2f}" for i, x in enumerate(np.linspace(0.63, 0.89, 20, endpoint=True)) if i%2==0])
    plt.colorbar()
    plt.xlabel(r"$H$")
    plt.ylabel(r"$H_\gamma$")
    ...
    plt.savefig(f"aucs_mean_contour.png", dpi=300, bbox_inches='tight')
    plt.close()

    # ------ PLOTTING -----------
    # Contour plot
    plt.contourf(param1, param2, AUCS_std.T, levels=15)
    # plt.title(f"AUCS")
    plt.colorbar()
    plt.xlabel(r"$H$")
    plt.ylabel(r"$H_\gamma$")
    plt.xticks(param1, param1)
    plt.yticks(param2, param2)
    plt.savefig(f"aucs_std_contour.png", dpi=300, bbox_inches='tight')
    plt.close()
