...
if __name__ == '__main__':
    ...
    for size in ['small', 'large']:
        filename = f"data_H_15_model_{size}.csv"
    ...
    # Create plots
    for df, size in zip(dfs, ['small', 'large']):
        ...
        for i, p1 in enumerate(param1):
            ...
            # Plot reward history
            ax4.plot(np.mean(metric, axis=1), label=r"$\nu=$" + f" {p1}", color=colors[i])
            ax4.fill_between(
        ...
        #ax4.set_xticks([2*k+1 for k in range(20)], [f"{(2*k+2)*10}k" for k in range(20)])
        fig4.savefig(f"reward_history_model_{size}.png", dpi=300, bbox_inches='tight')
        ...
        # Contour plot
        ax2.plot(param1, FINAL_mean, label=f"model size = {size}")
        ax2.fill_between(
            param1,
        ...
        min_param1 = param1[min_idx[0]]
        # Contour plot
        ax3.plot(param1, FINAL_AVG_mean, label=f"model size = {size}")
        ax3.fill_between(
            param1,
        ...
        min_param1 = param1[min_idx[0]]
        # Contour plot
        ax1.plot(param1, AUCS_mean, label=f"model size = {size}")
        ax1.fill_between(
            param1,
