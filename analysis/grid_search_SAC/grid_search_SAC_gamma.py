...
def load_runs_from_wandb_project(path):
    ...
    for run in tqdm(api.runs(path=path)):
        ...
        # print(run.config)
        run_parameters = []
        run_parameters.append(eval(run.config['optim'])['gamma'])
        for i, row in run.history(keys=["eval/mean_reward"]).iterrows():
            rewards.append(row["eval/mean_reward"])
        for i, row in run.history(keys=["eval/mean_last_reward"]).iterrows():
            last_rewards.append(row["eval/mean_last_reward"])
...
if __name__ == '__main__':
    ...
    project_path = "SAC_KS_gridsearch_gamma_nu001"
    hyperparams = ['gamma']

    # Load everything
    cfg = load_config_from_wandb_project(path=project_path)
    eval_steps = eval(cfg['logger'])['test_episode_length']
    nu = eval(cfg["env"])["nu"]
