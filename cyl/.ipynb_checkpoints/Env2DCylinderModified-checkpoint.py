...
class Env2DCylinderModified(gym.Env):
    ...
    def start_class(self):
       ...
       # Create ring buffers to hold recent history of jet values, probe values, lift, drag, area
        ...
        self.history_parameters["number_of_probes"] = copy.deepcopy(len(self.output_params["locations"]))  #############
        ...
        if self.n_iter_make_ready is None:
            ...
            # Load initial fields
            self.flow_params['u_init'] = '../cyl/mesh/u_init.xdmf'
            self.flow_params['p_init'] = '../cyl/mesh/p_init.xdmf'
            ...
            cwd = os.getcwd()
            print("test cwd env: ", cwd)
            with open('../cyl/mesh/dict_history_parameters.pkl', 'rb') as f:
            ...
            print("len(self.output_params[locations]): ", len(self.output_params["locations"]))
            print("self.history_parameters[number_of_probe]: ", self.history_parameters["number_of_probes"])
        ...
        if self.n_iter_make_ready is not None:
            ...
            # save field data
            XDMFFile(comm, '../cyl/mesh/u_init.xdmf').write_checkpoint(self.u_, 'u0', 0, encoding)
            XDMFFile(comm, '../cyl/mesh/p_init.xdmf').write_checkpoint(self.p_, 'p0', 0, encoding)

            # save buffer dict
            with open('../cyl/mesh/dict_history_parameters.pkl', 'wb') as f:
    ...
    def step(self, actions=None):
        ...
        #Logging reward, wake observation and state. Can be deactivated by commenting out.
